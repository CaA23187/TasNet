...
class Encoder(nn.Module):
    ...
    def forward(self, mixture):
        ...
        mixture_w = torch.relu(self.conv1d_U(mixture))  # [M, N, K]
        return mixture_w
...
class Decoder(nn.Module):
    ...
    def forward(self, mixture_w, est_mask):
        ...
        """
        # D = W * M
        # print(torch.unsqueeze(mixture_w, 1).size(), est_mask.size())
        mix = []
        for i in range(est_mask.size(1)):
            mix.append(mixture_w)
        a = torch.stack(mix, 1)
        # print(a.size(), est_mask.size())
        source_w = a * est_mask
...
class ChannelwiseLayerNorm(nn.Module):
    ...
    def forward(self, y):
        ...
        Returns:
            cLN_y: [M, N, K]
        """
        mean = torch.mean(y.float(), dim=1, keepdim=True)  # [M, 1, K]
        var = torch.var(y.float(), dim=1, keepdim=True, unbiased=False)  # [M, 1, K]
...
if __name__ == "__main__":
    ...
    # est_mask = torch.randn( (M,C,N,K))
    # est_source = decoder(mixture_w, est_mask)
    # print('est_source', est_source)
    #
    # # test Conv-TasNet
    # conv_tasnet = ConvTasNet(N, L, B, H, P, X, R, C, norm_type=norm_type)
    # # print('conv_tasnet', conv_tasnet)
    ...
    TasNet = ConvTasNet(N, L, B, H, P, X, R, C, norm_type="gLN", causal=True, mask_nonlinear='relu').to(DEVICE)
    print(TasNet)


    data = torch.randn([M, T]).to(DEVICE)
    t1 = time.time()
    out = TasNet(data)
    print(out,'\nout.size=',out.size())
    print(time.time() - t1)
